...
import styles from "./Campaign.module.scss";
import { QRCodeCanvas } from "qrcode.react";
import heroBg from "../../assets/iqra-hero.png";
import logoImg from "../../assets/iqra-hero.png";
import {
  demoCampaign,
  demoLeaderboard,
  demoRecentDonations,
...
// ✅ Change these to your real UPI details
const PAYEE_VPA = "yourvpa@upi"; // ex: ahamed@okicici
const PAYEE_NAME = "Knowledge World";
const CURRENCY = "INR";
...
export default function CampaignDemo() {
  ...
  // ✅ scroll target (Donate Now button)
  ...
  function downloadQrPngFromModal() {
    const root = modalQrRef.current;
    if (!root) return;
    ...
    const a = document.createElement("a");
    a.href = dataUrl;
    a.download = `upi-qr-${Number(form.amount || 0)}.png`;
    document.body.appendChild(a);
    a.click();
    a.remove();
  }
  ...
  function recomputeOverallLeaderboard(nextRecent) {
    ...
    setLb((prev) => ({
      ...prev,
      overall: sorted.length ? sorted : prev.overall,
    }));
  }
  ...
  function makeDonation({ status }) {
    ...
    const receiptNo = `RCP-${String(receiptCounterRef.current).padStart(4, "0")}`;
    const paymentId = status === "PAID" ? `PAY_DEMO_${makeId(6)}` : "";
  ...
  async function handleDonate(e) {
    ...
    // Pay Later: directly save pledge
    if (form.paymentMode === "payLater") {
      setPaying(true);
      await new Promise((r) => setTimeout(r, 650));

      const pledge = makeDonation({ status: "PLEDGED" });
      const nextRecent = [pledge, ...recent];
      setRecent(nextRecent);
      ...
      setMsg(`✅ Pledge saved! Receipt: ${pledge.receiptNo} (Pay later)`);
      setPaying(false);
      return;
    }

    // Pay Now: open modal (NO navigation)
  ...
  async function confirmPaid() {
    ...
    const paid = makeDonation({ status: "PAID" });
    const nextRecent = [paid, ...recent];
    setRecent(nextRecent);
    ...
    setLb((p) => {
      const nextToday = [
        { rank: 1, donorName: paid.donorName, total: paid.amount, count: 1 },
        ...p.today,
      ].slice(0, 10);
    ...
    setMsg(`✅ Demo payment saved! Receipt: ${paid.receiptNo} (No real payment)`);
    setPaying(false);
  }
...
// ✅ open UPI app (works mainly on mobile)
function openUpiApp(app) {
  const qs = upiQueryString(upiQrValue);

  const map = {
    gpay: `gpay://upi/pay?${qs}`,
    phonepe: `phonepe://pay?${qs}`,
    paytm: `paytmmp://pay?${qs}`,
  };

  window.location.href = map[app];
}
...
>
  ...
  <div className={styles.heroContent}>
    <div className={styles.heroTag}>Donate</div>
    <h1>We Need Your Help And Care</h1>
    <p>
      Your generosity helps provide quality education and safe learning spaces for
      underprivileged students. Every contribution counts.
    </p>
    <button
      type="button"
    ...
    >
      ...
      {/* ===== FEATURES (like website 3 boxes) ===== */}
      {/* <section className={styles.featureStrip}>
        <div className={styles.featureBox}>
          <h4>Empower Through Education</h4>
          <p>
            Provide learning resources and support for students to access quality
            education.
          </p>
        </div>
        ...
        <div className={styles.featureBox}>
          <h4>Nourish Hope</h4>
          <p>
            Help ensure children have meals and essentials that improve daily
            learning.
          </p>
        </div>

        <div className={styles.featureBox}>
          <h4>Build Safe Spaces</h4>
          <p>
            Support safe classrooms and better facilities for a secure environment.
          </p>
        </div>
      </section> */}
...
>
  <div className={styles.left}>
    <div className={styles.headerTop}>
      <img src={heroBg} alt="IQRA Logo" className={styles.headerLogo} />
      <div>
        <h2>{campaign.title}</h2>
        <p>{campaign.description}</p>
      </div>
          ...
          <form onSubmit={handleDonate} className={styles.form}>
            ...
            <label className={styles.check}>
              <input
                ...
                onChange={(e) => setForm((p) => ({ ...p, isAnonymous: e.target.checked }))}
              />
              Hide your details(particate without revealing your details)
            </label>

            <button className={styles.btn} disabled={paying}>
              {paying
                ? "Processing..."
                : form.paymentMode === "payLater"
                ? "Submit Pledge"
                : "Pay & Donate"}
            </button>
            ...
            <div className={styles.note}>
              Pay Now opens UPI QR popup. Pay Later saves pledge only.
            </div>
          </form>
        </div>
  ...
  >
    <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
      ...
      <div className={styles.modalBody}>
        {/* QR */}
        <div className={styles.qrBox} ref={modalQrRef}>
          <QRCodeCanvas value={upiQrValue} size={170} includeMargin />
        </div>
        ...
        <div className={styles.modalActions}>
          ...
          <button
            ...
            onClick={() => copyText(PAYEE_VPA, "✅ UPI ID copied")}
          >
            Copy UPI ID
          </button>
          ...
          <button
            type="button"
            className={styles.actionBtn}
            onClick={() => copyText(upiQrValue, "✅ UPI Link copied")}
          >
            Copy UPI Link
          </button>
          ...
          <button
            type="button"
            className={styles.actionBtn}
            onClick={downloadQrPngFromModal}
          >
            Download QR (PNG)
          </button>
      ...
      <div className={styles.modalFooter}>
        ...
        <button
          ...
          onClick={confirmPaid}
        >
          {paying ? "Verifying..." : "I Paid"}
        </button>
      </div>

      <div className={styles.modalNote}>
        Demo: Clicking “I Paid” records donation instantly (no real payment).
      </div>
    </div>
  </div>
)}
